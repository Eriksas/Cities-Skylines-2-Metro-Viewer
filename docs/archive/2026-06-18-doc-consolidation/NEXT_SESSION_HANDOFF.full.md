...
- 2026-06-14 station-name semantics follow-up: default CS2 asset names are now separated from likely player-authored station names. Optional virtual transfer hints are available through CLI `--enable-virtual-transfer-hints` and the Viewer checkbox `Show virtual transfer hints` / `显示虚拟换乘提示`; the feature is off by default and ignores known asset/default names.
- 2026-06-14 medium-term schematic follow-up: `schematic-map` is now the product-facing official-map style entry built on the schematic-v2 topology engine. It applies transit-map framing, path geometry, service family merge, and express center stripes by default. Keep `geographic` as the alpha recommended baseline; use `schematic-v2` for lower-level diagnostics and `schematic-lite` only from CLI for historical comparison.
- 2026-06-15 simple-city validation follow-up: the Sheffield export (`D:\CS2MetroDiagram\exports\metro-export-谢菲尔德-20260614-233659.json`) generated a simple-city alpha bundle at `artifacts\alpha-validation\20260615-104939-sheffield-simple-city-manifest`. `generate-alpha-validation-bundle.ps1` now includes `schematic-map.svg`, `schematic-map.full.png`, and `manifest.json`, making the product-facing map and machine-readable review metadata part of every feedback bundle.
- 2026-06-15 compact-frame follow-up: `schematic-map` now uses a compact transit-map frame for small simple networks (1-4 display families, up to 40 stations, no service variants). The standard frame remains for complex/service-variant maps. Latest Sheffield check:
  - `artifacts\product-candidate\20260615-111148-sheffield-compact-frame-final`
  - Verified with `dotnet build CS2MetroDiagram.slnx --no-restore` and `dotnet run --project src\MetroDiagram.Tests\MetroDiagram.Tests.csproj --no-restore`.
- 2026-06-15 schematic-map octilinear follow-up: product-facing `schematic-map` now normalizes route segments that are already close to horizontal, vertical, or 45 degrees. The pass is renderer-only, does not affect `geographic` or diagnostic `schematic-v2`, and keeps interchange/high-degree nodes anchored.
  - Sheffield candidate: `artifacts\product-candidate\20260615-230949-sheffield-octilinear-normalization`
  - Zhaoqing candidate: `artifacts\product-candidate\20260615-231237-zhaoqing-octilinear-normalization`
  - Verification passed with `dotnet build CS2MetroDiagram.slnx --no-restore` and `dotnet run --project src\MetroDiagram.Tests\MetroDiagram.Tests.csproj --no-restore`.
  - Next visual check: compare these candidates against the latest Viewer output, especially 2号线/10号线 shared corridor visibility, west-side 3号线/4号线 visibility, and whether simple-city long lines read as cleaner H/V/45 schematic segments.
- 2026-06-15 schematic-map local-clearance follow-up: product-facing `schematic-map` now gives ordinary stations a little clearance from unrelated route segments. This addresses the Sheffield 1号线 / 3号线 crowded area without changing topology or moving interchange/high-degree anchors.
  - Sheffield clearance candidate: `artifacts\product-candidate\20260615-232626-sheffield-local-clearance`
  - Zhaoqing regression candidate: `artifacts\product-candidate\20260615-232722-zhaoqing-local-clearance-regression`
  - Diagnostics now include `schematic-map clearance stations: N`.
  - Verification passed with `dotnet run --project src\MetroDiagram.Tests\MetroDiagram.Tests.csproj --no-restore` and `dotnet build CS2MetroDiagram.slnx --no-restore`.
- 2026-06-17 schematic-map simple-run linearization follow-up: product-facing `schematic-map` now straightens ordinary non-anchor route runs into cleaner horizontal, vertical, or 45-degree schematic strokes with more even station spacing.
  - This is intentionally more abstract than diagnostic `schematic-v2`.
  - It is renderer-only and does not change exporter output, JSON schema, geographic output, raw `line.stops`, or raw `line.pathPoints`.
  - New diagnostics field: `schematic-map linearized stations: N`.
  - New synthetic test: `schematic-map linearizes simple ordinary route runs`.
  - Sheffield candidate: `artifacts\product-candidate\20260617-132923-sheffield-abstract-linear-runs`
  - Zhaoqing regression candidate: `artifacts\product-candidate\20260617-133019-zhaoqing-abstract-linear-runs-regression`
  - Next visual checks: confirm simple-city ordinary lines feel more official-map-like, then re-check Zhaoqing `2号线` / `10号线` shared corridor and west-side `3号线` / `4号线` visibility before accepting additional abstraction passes.
- 2026-06-17 schematic-map station-spacing hierarchy follow-up: simple-run linearization now preserves genuinely long adjacent station gaps while keeping ordinary station gaps rhythmically consistent.
  - New test: `schematic-map preserves genuinely long station gaps`.
  - Sheffield candidate: `artifacts\product-candidate\20260617-134847-sheffield-spacing-hierarchy`
  - Zhaoqing regression candidate: `artifacts\product-candidate\20260617-134848-zhaoqing-spacing-hierarchy-regression`
  - Quick visual read: Sheffield is cleaner and more official-map-like; Zhaoqing remains usable with no obvious regression in the sensitive shared-corridor areas.
- 2026-06-17 schematic-map crossing-bridge follow-up: product-facing `schematic-map` now marks true non-station X crossings with a small bridge/gap marker rather than treating them as ordinary direct intersections.
  - The crossing audit skips station/interchange crossings, segment endpoints, same-family intersections, and near-parallel overlaps.
  - New tests:
    - `schematic-map marks non-station route crossings with bridge markers`
    - `schematic-map does not bridge station crossings`
  - Sheffield candidate: `artifacts\product-candidate\20260617-155117-sheffield-crossing-bridges`
  - Zhaoqing regression candidate: `artifacts\product-candidate\20260617-155142-zhaoqing-crossing-bridges-regression`
  - Verified with `dotnet run --project src\MetroDiagram.Tests\MetroDiagram.Tests.csproj --no-restore` and `dotnet build CS2MetroDiagram.slnx --no-restore`.
  - Next visual checks: confirm the bridge marker reads naturally at map scale; if repeated cities show too many bridges, consider a later true crossing-avoidance route nudge rather than expanding this marker layer.
- 2026-06-18 new Sheffield export review:
  - Input: `D:\CS2MetroDiagram\exports\metro-export-谢菲尔德-20260617-223135.json`
  - Summary: 8 display families, 43 stations, 12580 pathPoints.
  - Product candidate: `artifacts\product-candidate\20260618-121615-sheffield-new-export-review`
  - Alpha validation bundle: `artifacts\alpha-validation\20260618-122028-sheffield-new-export-review`
  - `schematic-map` review: no dense station pairs, 2 non-station crossings bridged, no obvious blocking topology/visibility regression.
  - Script fix: `generate-alpha-validation-bundle.ps1` now prefers `pwsh` for helper scripts; this fixed the `schematic-map.full.png` capture failure from the first bundle attempt.
  - Next manual step: inspect `schematic-map.full.png` and `baseline-geographic.full.png` from the bundle. If the two bridge markers feel too visually heavy, tune bridge marker presentation only; do not change exporter/schema/geographic output.
- 2026-06-18 alpha validation index:
  - Added `scripts\summarize-alpha-validation-bundles.ps1`.
  - Current outputs:
    - `artifacts\alpha-validation\index.md`
    - `artifacts\alpha-validation\index.csv`
  - Use this after each new real-city bundle to see city names, line/station counts, PNG availability, and missing-manifest legacy bundles in one table.
  - Current index includes 4 bundle directories; two older bundles do not have `manifest.json` and should be regenerated before formal alpha review.
