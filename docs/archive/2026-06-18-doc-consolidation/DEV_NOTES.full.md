...
## Schematic-map simple run linearization - 2026-06-17

- Product-facing `schematic-map` now has a simple-run linearization pass that is deliberately more abstract than diagnostic `schematic-v2`.
- The pass only runs for `schematic-map`; it does not affect `geographic`, `schematic-v2`, exporter output, JSON schema, raw `line.stops`, or raw `line.pathPoints`.
- Ordinary route runs between conservative anchors can be straightened into horizontal, vertical, or 45-degree strokes with more even station spacing.
- Anchors are kept conservative: interchanges, high-degree stations, and multi-family stations are protected so topology and shared-corridor relationships remain stable.
- Diagnostics now include `schematic-map linearized stations: N`, and adjusted station markers can include `data-schematic-station-adjustment-reason="schematic-map-simple-run-linearization"`.
...
- Sheffield review note: the simple 3-line city reads much closer to an official schematic; long ordinary lines are cleaner and station spacing is more regular.
- Zhaoqing regression note: the map remains usable after abstraction; existing `2号线` / `10号线` and `3号线` / `4号线` visibility relationships should continue to be checked after every schematic-map polish pass.

## Schematic-map station spacing hierarchy - 2026-06-17

- Simple-run linearization now computes per-segment schematic spacing instead of forcing every adjacent station pair to the same interval.
- Ordinary station gaps still normalize to a common visual rhythm, but a segment that is clearly much longer than the local median can retain a longer schematic interval.
- This keeps the product map closer to real transit diagrams: dense ordinary stops read evenly, while airport/suburban-style long links do not disappear.
- The change is render-only and applies only to `schematic-map`.
- New test: `schematic-map preserves genuinely long station gaps`.
...
- Visual review:
  - Sheffield: the simple network is cleaner and more official-map-like, with the long green corridor still reading as a long outer segment.
  - Zhaoqing: no obvious regression in the already-sensitive `2号线` / `10号线` and `3号线` / `4号线` areas during quick review.
...
## Station-name semantics and virtual transfer hints - 2026-06-14

- `StationLabelClassifier` now separates station names into:
  - `Fallback` (`Station 1`, blank names, or station-id fallbacks),
  - `KnownAssetDefault` (`现代地铁站`, `小型地铁广场`, `地下地铁站`, `Metro Station`, etc.),
  - `UserNamed` (everything else).
- `HideGenericStationLabels` still hides fallback and known asset/default names. It does not hide likely player-authored station names.
- Schematic-v2 dense-station diagnostics now distinguish:
  - `same-name asset-default clusters`,
  - `same-name likely-user clusters`.
- Added opt-in virtual transfer hints:
  - CLI: `--enable-virtual-transfer-hints`
  - Viewer checkbox: `Show virtual transfer hints` / `显示虚拟换乘提示`
  - Renderer option: `SvgRenderOptions.EnableVirtualTransferHints`
- The hint renderer draws a gray dashed connector between nearby same-name stations only when the shared name is classified as `UserNamed`.
- Known asset/default names such as `现代地铁站` are intentionally ignored for virtual transfer hints, because repeated asset labels usually mean unrenamed/default stations rather than intentional out-of-station transfer.
- Research note: virtual/out-of-station transfer is generally represented as a special/dashed walking or external transfer connector, not as a normal in-station interchange marker. Keep the feature off by default until the player explicitly enables it.

## Schematic-map product entry - 2026-06-14

- Added a new renderer layout mode:

```text
schematic-map
```

- `schematic-map` is the medium-term product-facing schematic entry. It reuses the topology-first `schematic-v2` engine but applies official-map presentation defaults:
  - transit-map frame/header/key,
  - path geometry enabled,
  - service family merge enabled,
  - express service white center stripe enabled.
- `schematic-v2` remains the experimental/diagnostic base. `schematic-lite` remains CLI-only historical comparison.
- The Viewer now lists `schematic-map` as a selectable layout. Saved `schematic-v2` settings remain valid.
- Keep future official-map polish in `schematic-map` unless the lower-level topology engine itself needs a fix. Do not change exporter output, JSON schema, raw `line.stops`, raw `line.pathPoints`, or the geographic alpha baseline for this work.

## Schematic-map visual baseline polish - 2026-06-14

- `schematic-map` now applies stronger product presentation defaults than raw `schematic-v2`:
  - larger station/interchange marker defaults,
  - larger minimum station-label size,
  - wider label gap,
  - larger route badge typography,
  - more conservative route badge buffers around station markers and placed labels,
  - slightly taller transit-map bottom key reserve.
- The transit-map header now uses the city name in the main title when available, for example `肇庆线网示意图`.
- In `schematic-map`, the small left-side duplicate city/title label is hidden because the city name is already in the main header.
- The bottom key now includes compact service-variant notes for families with multiple variants so express/local metadata is visible in the map artifact, not only in debug attributes.
- `scripts\generate-product-candidate-map.ps1` now defaults to `-Layout schematic-map`, while still allowing `geographic` or `schematic-v2` when comparison output is needed.
- These settings are applied only when `LayoutMode == SchematicMap`; `geographic`, `schematic-v2`, and CLI-only `schematic-lite` keep their existing behavior.
- This is a visual baseline polish pass, not a topology solver change. If lines separate, overlap, or route incorrectly, diagnose the schematic-v2 topology/corridor layer rather than tuning these visual tokens.

## Simple-city alpha validation bundle - 2026-06-14

- `generate-alpha-validation-bundle.ps1` now includes `schematic-map.svg` and `schematic-map.full.png` in each bundle, alongside geographic, schematic-lite, schematic-v2, diagnostics, notes, and the filled feedback template.
- The same bundle now writes `manifest.json` with city/export metadata, tool version, generated file names, render settings, validation warnings, and a recommended review order. Prefer this manifest when comparing multiple city feedback bundles.
- A simple-city smoke bundle was generated from the latest Sheffield export:

```text
D:\CS2MetroDiagram\exports\metro-export-谢菲尔德-20260614-233659.json
artifacts\alpha-validation\20260615-104939-sheffield-simple-city-manifest
```

- Export summary: city `谢菲尔德`, 3 lines, 19 stations. The bundle is useful for catching product-map regressions in title/header, bottom key, route badges, station markers, and basic label readability without the dense-primary-city noise.

## Schematic-map compact frame - 2026-06-15

- `schematic-map` now automatically switches to a compact transit-map frame for small simple networks:
  - 1-4 display families,
  - 40 or fewer stations,
  - no service variants.
- The compact frame reduces header/footer reserve while keeping the product header and bottom key, so small exports do not feel like sparse large-city maps.
- Complex maps and maps with service variants keep the standard frame because the key needs more space.
- Current simple-city check output:

```text
artifacts\product-candidate\20260615-111148-sheffield-compact-frame-final
```

## Viewer preview content focus - 2026-06-15

- The embedded Viewer preview now focuses on SVG content bounds instead of always showing the full exported canvas from `(0,0)`.
- The preview HTML computes a render-time bbox from route, route badge, station, label, virtual-transfer, and non-bottom legend groups, then adjusts the preview-only SVG `viewBox` and pixel size.
- This is Viewer-only. Saved SVG output remains unchanged, and `schematic-map` bottom keys are intentionally excluded from the focus bbox so small maps do not regain a large blank preview area.

## Schematic-map octilinear normalization - 2026-06-15

- `schematic-map` now applies a conservative render-time octilinear normalization pass after terminal-tail straightening and sharp-angle relaxation.
- The pass is enabled only for `SvgLayoutMode.SchematicMap`; `geographic`, diagnostic `schematic-v2`, and CLI-only `schematic-lite` keep their existing behavior.
- The pass snaps ordinary low-risk station positions when their adjacent route segments are already close to horizontal, vertical, or 45-degree directions.
- Interchange and high-degree stations are treated as anchors so the pass does not disturb fixed topology hubs or already validated shared-corridor cases.
- The warning summary includes `schematic-map octilinear stations: N` when stations are adjusted.
- `schematic-map` also applies a narrow local-clearance pass after octilinear normalization. It pushes ordinary low-risk stations away from unrelated route segments when they are visually too close.
- The local-clearance pass is deliberately not a general force-directed layout. It does not move interchange/high-degree anchors and does not operate on `geographic` or diagnostic `schematic-v2`.
- The warning summary includes `schematic-map clearance stations: N` when this pass adjusts stations.
- Generated validation candidates:

```text
artifacts\product-candidate\20260615-230949-sheffield-octilinear-normalization
artifacts\product-candidate\20260615-231237-zhaoqing-octilinear-normalization
artifacts\product-candidate\20260615-232626-sheffield-local-clearance
artifacts\product-candidate\20260615-232722-zhaoqing-local-clearance-regression
```
...
## Schematic-map crossing bridge markers - 2026-06-17

- Added a renderer-only crossing audit for `schematic-map`.
- The audit detects true non-station route segment intersections and draws a small bridge/gap marker on the later/top route so a visual X crossing does not read as a false interchange.
- The audit deliberately skips same-family intersections, near-parallel or overlap segments, segment endpoints, and crossings close to station/interchange markers.
- SVG debug attributes include:

```text
data-schematic-map-crossing-bridge="true"
data-schematic-map-crossing-layer="gap|top"
data-schematic-map-crossing-family-top="..."
data-schematic-map-crossing-family-bottom="..."
```

- Generated product candidates:

```text
artifacts\product-candidate\20260617-155117-sheffield-crossing-bridges
artifacts\product-candidate\20260617-155142-zhaoqing-crossing-bridges-regression
```
...
## New Sheffield export review - 2026-06-18

- Reviewed the latest Sheffield snapshot:

```text
D:\CS2MetroDiagram\exports\metro-export-谢菲尔德-20260617-223135.json
```

- Export summary:
  - city: `谢菲尔德`
  - display/source lines: 8
  - stations: 43
  - lines with pathPoints: 8
  - total pathPoints: 12580
- Generated product candidate:

```text
artifacts\product-candidate\20260618-121615-sheffield-new-export-review
```

- Generated alpha validation bundle after fixing screenshot runner:
...
- Findings:
  - `schematic-map` has no dense station pairs on this export.
  - `schematic-map` reports 18 linearized stations, 8 octilinear adjustments, 5 local-clearance adjustments, and 2 bridged non-station crossings.
  - Visual review did not show a blocking topology or visibility regression.
- Script fix:
  - `scripts\generate-alpha-validation-bundle.ps1` now prefers `pwsh` for helper scripts and falls back to Windows PowerShell.
  - This fixed a `schematic-map.full.png` capture failure that occurred when the bundle script used `powershell.exe`, while the product-candidate script already succeeded with `pwsh`.

## Alpha validation bundle index - 2026-06-18
...
- The script scans bundle directories, reads `manifest.json` when available, and records:
  - city name,
  - line/station counts,
  - generated time,
  - source export path,
  - PNG availability for geographic / schematic-map / schematic-v2,
  - validation warnings.
- Older bundles without `manifest.json` are still listed with `missing manifest.json`, so they can be regenerated before formal review.
- Current run indexed 4 bundles, including:
  - `20260618-122028-sheffield-new-export-review`
  - `20260615-104939-sheffield-simple-city-manifest`
