...
## 2026-06-17 - Make schematic-map more abstract than schematic-v2

Decision: Add a product-facing simple-run linearization pass to `schematic-map`, while keeping `schematic-v2` as the diagnostic/topology layout.

Reason: Real transit diagrams often simplify ordinary service runs into straight horizontal, vertical, or 45-degree strokes with more even station spacing. Continuing to tune only local geographic deviations would keep the output looking like a technical graph instead of an official map. The new pass is renderer-only, protects interchanges/high-degree/multi-family anchors, and does not change exporter output, JSON schema, raw `line.stops`, raw `line.pathPoints`, or `geographic` output.

Follow-up: Station spacing should be rhythmic but not completely uniform. `schematic-map` now preserves genuinely long adjacent station gaps while normalizing ordinary gaps. This keeps airport/suburban distance cues without returning to geographic geometry tracing.
...
## 2026-06-14 - Keep virtual transfer hints opt-in and user-name based

Decision: Add renderer-only virtual transfer hints behind `SvgRenderOptions.EnableVirtualTransferHints`, CLI `--enable-virtual-transfer-hints`, and a Viewer checkbox. The hint detector only connects nearby same-name stations when the shared name is classified as likely player-authored.

Reason: CS2 default station asset names such as `现代地铁站` and `小型地铁广场` can repeat many times without representing a meaningful place or transfer. Player-authored repeated names may intentionally describe virtual transfer, parallel platforms, or nearby related stations. Keeping dashed transfer hints opt-in avoids surprising users while still supporting deliberate virtual interchange review.

Consequence: No exporter, JSON schema, geographic baseline, or raw `station.position` / `line.stops` data changes. Known asset/default names remain generic labels and never generate virtual transfer hints.

## 2026-06-14 - Add schematic-map as the product-facing schematic mode

Decision: Add `schematic-map` as a new renderer layout mode and Viewer/CLI option, built on the existing schematic-v2 topology engine.

Reason: The user-facing goal is closer to an official metro system map, while `schematic-v2` has become a useful but technical diagnostics name. `schematic-map` gives the product direction a stable entry point: transit-map frame, path geometry, service family merge, and express service center stripes are applied by default without replacing or hiding the lower-level schematic-v2 base.

Consequence: `geographic` remains the alpha recommended baseline, `schematic-v2` remains available for diagnostics/experiments, and `schematic-lite` remains CLI-only historical comparison. Exporter output, JSON schema, raw stops, raw pathPoints, and geographic rendering are unchanged.

## 2026-06-14 - Keep schematic-map polish in presentation tokens first

Decision: Phase 5F.1 improves `schematic-map` through visual presentation defaults before changing any schematic topology logic.

Reason: The current medium-term goal is to make the official-map candidate look more intentional without destabilizing already fixed corridor cases such as `2号线` / `10号线`, west-side `3号线` / `4号线`, and `10号线` shared exact-platform overlays. Station/label/badge/key-panel readability is a safer next layer than another route-chain pass.

Consequence: `schematic-map` now uses stronger product defaults for marker size, label size/gap, route badge typography, route badge collision buffers, and bottom key reserve. Other layout modes and exporter/schema contracts are unchanged.

## 2026-06-15 - Use a compact transit-map frame for small simple schematic maps

Decision: `schematic-map` now automatically chooses a compact transit-map frame for small simple networks (1-4 display families, 40 or fewer stations, and no service variants).

Reason: The Sheffield simple-city export showed that the standard large-network frame made a small map feel sparse and over-packaged. Compacting the header/footer reserve improves the product-map composition without changing route topology, station data, exporter behavior, or JSON schema.

Consequence: Small simple `schematic-map` outputs include `data-transit-map-frame="compact"`. Complex maps and maps with service variants keep `data-transit-map-frame="standard"` so service notes and the bottom key retain enough room.

## 2026-06-15 - Normalize near-octilinear schematic-map segments

Decision: `schematic-map` now has a renderer-only octilinear normalization pass that snaps ordinary stations toward horizontal, vertical, and 45-degree segment directions when the existing route direction is already close.

Reason: The product-map target should read more like a real metro schematic, where most route segments are horizontal, vertical, or diagonal. This should be done conservatively: a small visual deviation can be normalized, but the pass should not invent new topology or force every geographic diagonal into an official-map direction.

Consequence: The pass is limited to `schematic-map`, leaves `geographic` and diagnostic `schematic-v2` behavior unchanged, keeps raw stops/pathPoints untouched, and treats interchange/high-degree nodes as anchors. If a future city needs stronger layout changes, that should become a dedicated schematic solver improvement rather than more presentation-token tuning.

## 2026-06-15 - Add local clearance for schematic-map route breathing room

Decision: `schematic-map` now has a narrow local-clearance pass that moves ordinary low-risk stations away from unrelated route segments when they are visually too close.

Reason: The Sheffield simple-city output showed that lines can be topologically correct and octilinear but still feel crowded when two unrelated routes run through the same visual band. A small automatic clearance pass gives the map more breathing room without changing service data or route topology.

Consequence: The pass is renderer-only and limited to `schematic-map`. It skips interchange/high-degree anchors, does not apply to `geographic` or diagnostic `schematic-v2`, and should be treated as a product polish layer rather than a full layout solver.

## 2026-06-17 - Mark non-station schematic-map crossings with bridge symbols

Decision: Add renderer-only bridge/gap markers for true non-station X crossings in `schematic-map`.

Reason: Some product-map candidates are topologically valid but still look unlike a transit schematic when two unrelated route strokes cross directly. A small overpass-style marker communicates that the routes cross without interchanging, while avoiding another broad route-layout rewrite.

Consequence: This does not move stations or route geometry, and it does not affect `geographic`, `schematic-v2`, exporter output, JSON schema, raw stops, or raw pathPoints. Station/interchange crossings are skipped so station markers remain the topology signal.

## 2026-06-18 - Track alpha city review through bundle manifests

Decision: Add a bundle index script for alpha validation outputs instead of relying on manual artifact-folder memory.

Reason: Multi-city validation is becoming the next meaningful quality gate. A manifest-backed index makes it clear which cities have complete geographic, schematic-map, schematic-v2, notes, and feedback artifacts, and it flags older bundles that should be regenerated.

Consequence: `scripts\summarize-alpha-validation-bundles.ps1` writes `artifacts\alpha-validation\index.md` and `index.csv`. This does not change renderer behavior, exporter output, JSON schema, or release packaging.
