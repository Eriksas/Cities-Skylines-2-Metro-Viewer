...
export function prepareInlineSvg(svg) {
  ...
  return withoutDocumentHeaders.replace(/<svg\b([^>]*)>/i, (_match, attributes) => {
    const responsiveAttributes = attributes
      ...
      .replace(/\sstyle=("[^"]*"|'[^']*')/i, "");
    return `<svg${responsiveAttributes} width="100%" height="100%" preserveAspectRatio="xMidYMid meet" style="display:block;width:100%;height:100%;font-family:var(--fontFamily)">`;
  });
}
...
function MapViewport({ svg, state, text }) {
  ...
  const moveDrag = React.useCallback((clientX, clientY) => {
    ...
    setOffset({
      x: drag.current.originX + clientX - drag.current.x,
      y: drag.current.originY + clientY - drag.current.y,
    });
  }, []);
