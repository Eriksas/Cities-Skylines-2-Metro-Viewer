...
Current version: `v0.1.0-alpha.3` · Alpha software — export only, it does not change your city. Back up saves before testing any mod.
...
当前版本：`v0.1.0-alpha.3` · Alpha 阶段软件 —— 只导出数据，不会改动你的城市。测试任何 mod 前请先备份存档。
...
1. 安装 mod（在此仅需要下载releases打包好的包解压即可），启动 CS2，载入城市。
